...
var utils = {
	...
	//拼接psrc串
	joinPsrc: function (arr, expression) {
		var psrc = '';
		for (var i = 0, j = arr.length; i < j; i++) {
		    arr[i] = returnSpecialChar(arr[i]);
		}
		psrc = arr.join(expression || '->');
		var refer = getKey(window.location.href, 'refer');
		if (!refer) {
		    refer = 0;
		}
		var referName = staticData.refer[refer];
		if (referName) {
		    psrc = referName + (expression || '->') + psrc;
		}
		return psrc;
	},

	// 获取 图片域名的随机数
	getImgNumber: function(pic) {
	...
	//通过歌曲或者专辑图片地址获取全路径
	getPic: function(pic, type, srcSize, desSize) {
	    pic = pic || '';
	    var picStr = location.protocol + '//kwimg' + this.getImgNumber(pic) + '.kuwo.cn/star/';
	    if (type == 'album') {
	        picStr += 'albumcover';
	    } else if (type == 'artist') {
	        picStr += 'starheads';
